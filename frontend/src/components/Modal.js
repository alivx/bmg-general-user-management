...
class Modal extends Component {
	...
	render () {
		...
		if (this.state.wasClickedLeft == false && this.state.wasClickedRight == false) {
			modalContent = (
				<div className='Modal'>
					<Sign type='delete' onChange={this.onClickLeft}></Sign>
					<Sign type='add' onChange={this.onClickRight}></Sign>
				</div>
			);
		} else if (this.state.wasClickedLeft == false && this.state.wasClickedRight == true) {
			modalContent = (
				<div className='Modal'>
					<SignCollapsed type='delete' onChange={this.onClickLeft}></SignCollapsed>
					<SignExpanded type='add' ></SignExpanded>
				</div>
			);
		} else if (this.state.wasClickedLeft == true && this.state.wasClickedRight == false) {
			modalContent = (
				<div className='Modal'>
					<SignExpanded type='delete' ></SignExpanded>
					<SignCollapsed type='add' onChange={this.onClickRight}></SignCollapsed>
				</div>
			);
		}
